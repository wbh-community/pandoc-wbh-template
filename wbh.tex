...
% Created:
% Changed: 26.06.2020
\PassOptionsToPackage{pdfencoding=auto,unicode,pdfa$for(hyperrefoptions)$,$hyperrefoptions$$endfor$}{hyperref}
...
% ------------------------------------------------------------
\usepackage[$if(geometry)$$for(geometry)$$geometry$$sep$,$endfor$$else$a4paper,margin=2cm,bottom=2.5cm,top=25mm,left=25mm,right=40mm,headsep=10mm,footskip=12mm$endif$]{geometry}
...
\def \assignment{
    ...
    % Check if assignment.file is provided
    $if(assignment.file)$

        % Check if the form should be filled out
        $if(assignment.fillform)$
            ...
            \includepdf[pagecommand={
                ...
                % Nodes for student and studium information
                \node at ($if(positions.student_firstname)$$positions.student_firstname$$else$34,-38$endif$) [info] {$student.firstname$};
                \node at ($if(positions.student_lastname)$$positions.student_lastname$$else$95,-38$endif$) [info] {$student.lastname$};
                \node at ($if(positions.student_strasse)$$positions.student_strasse$$else$34,-52$endif$) [info] {$student.strasse$};
                \node at ($if(positions.student_ort)$$positions.student_ort$$else$95,-52$endif$) [info] {$student.ort$};
                \node at ($if(positions.student_matrikelnr)$$positions.student_matrikelnr$$else$34,-65$endif$) [info] {$student.matrikelnr$};
                \node at ($if(positions.studium_studiengangnr)$$positions.studium_studiengangnr$$else$95,-65$endif$) [info] {$studium.studiengangnr$};
                \end{tikzpicture}
            }, pages=1]
            {$assignment.file$}

            % Check if multiple pages are needed, and if so, include the rest
            $if(assignment.multipage)$
                \includepdf[pages=$assignment.pages$]{$assignment.file$}
            $endif$

        $else$
            % Simply include the assignment without the form fill
            \includepdf[pages=$assignment.pages$]{$assignment.file$}
        $endif$

    $else$
        % Log that no assignment.file was provided (currently silent)
    $endif$
...
%\ifxetex %if xetex
	%pdfproducer={xetex \the\XeTeXversion\XeTeXrevision},
%\fi
\ifluatex %if luatex
	pdfproducer={luatex \the\luatexversion\luatexrevision },
\fi
...
% Verzeichnisse
% --------------------------------------------------------------
\lhead{$reference-section-title$}
\markboth{$reference-section-title$}{$reference-section-title$} % change the header for the bibliography chapter, maching the chapter title
