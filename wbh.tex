...
\thispagestyle{empty}
\begin{center}
$if(logo)$ $if(hochschule)$ % beides gesetzt
$else$   \includegraphics[max width=\textwidth]{$logo$}\\ $endif$  $endif$
$if(hochschule)$ $if(logo)$  % beides gesetzt
$else$   \Huge \textbf{$hochschule$} \\ $endif$   $endif$
    \vspace*{2cm}
	\Large
	$if(studium.studiengang)$
		\textbf{Studiengang:}\\
